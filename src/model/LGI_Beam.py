...
class Single_LGI_Beam(AbstractNetwork):
    ...
    def reset_status(self, init_reset=False):
        ...
        """
        super(Single_LGI_Beam, self).reset_status(init_reset=init_reset)
    ...
    def bring_dataset_info(self, dset):
        super(Single_LGI_Beam, self).bring_dataset_info(dset)
...
class LGI_Beam(torch.nn.Module):
    """
    ...
    def __init__(self,config, logger=None, verbose=True, device_ids=None, output_device=None):
        super(LGI_Beam,self).__init__()
        self.module = Single_LGI_Beam(config,logger=logger,verbose=verbose)

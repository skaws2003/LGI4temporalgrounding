...
class Single_LGI_CR(AbstractNetwork):
    ...
    def _build_network(self):
        ...
        self.criterion = bb.MultipleCriterions(
            ["grounding","CR"],
            [bb.TGRegressionCriterion(mconfig, prefix="grounding"),bb.CRLoss(mconfig,prefix="CRLoss")]
        )
    ...
    def _infer(self,net_inps,mode="forward",gts=None):
        if self.training:
            return self._infer_train(net_inps=net_inps,mode=mode,gts=gts)
        else:
            return self._infer_test(net_inps=net_inps,mode=mode,gts=gts)
    ...
    def _infer_test(self, net_inps, mode="forward", gts=None):
        # fetch inputs
    ...
    def prepare_batch(self, batch):
        self.gt_list = ["vids", "qids", "aug_qids", "timestamps", "duration",
    ...
    def reset_status(self, init_reset=False):
        ...
        """
        super(Single_LGI_CR, self).reset_status(init_reset=init_reset)
    ...
    def bring_dataset_info(self, dset):
        super(Single_LGI_CR, self).bring_dataset_info(dset)
...
class LGI_CR(torch.nn.Module):
    ...
    def __init__(self,config, logger=None, verbose=True, device_ids=None, output_device=None):
        super(LGI_CR,self).__init__()
        self.module = Single_LGI_CR(config,logger=logger,verbose=verbose)
    ...
    def scatter(self,batch):
        batch_size = len(batch['query_labels'])
        ...
        idxs = [(round(each_size * i), round(each_size * (i+1))) for i in range(device_cnt)]
        for i, (st, ed) in enumerate(idxs):
            device_batch = {}
            for k in batch.keys():
                device_batch[k] = batch[k][st:ed].to(self.device_ids[i]) \
                    if isinstance(batch[k], torch.Tensor) else batch[k][st:ed]
            new_batch.append(device_batch)
        return new_batch
    ...
    def bring_dataset_info(self,*args,**kwargs):
        self.module.bring_dataset_info(*args,**kwargs)

    def model_specific_config_update(self,*args,**kwargs):
        return self.module.model_specific_config_update(*args,**kwargs)

    def gpu_mode(self):
        self.module.gpu_mode()
    
    def print_status(self):
        self.module.print_status()
    ...
    def get_lr(self):
        return self.module.get_lr()
    
    def print_counters_info(self,*args,**kwargs):
        self.module.print_counters_info(*args,**kwargs)

    def create_tensorboard_summary(self,*args,**kwargs):
        self.module.create_tensorboard_summary(*args,**kwargs)

    def eval_mode(self):
        self.module.eval_mode()

    def reset_counters(self):
        self.module.reset_counters()

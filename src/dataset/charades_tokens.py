...
def create_loaders(split, loader_configs, num_workers):
    ...
    for di,dt in enumerate(split):
        ...
        dsets[dt] = CharadesTokensDataset(loader_configs[di])
...
class CharadesTokensDataset(AbstractDataset):
    ...
    def _get_data_path(self, config):
        ...
        root_dir = os.path.join(config.get("data_dir", ""), "preprocess_tok_unsup_coco_cnndm_yolov3spp")
    ...
    def generate_labels(self, config):
        ...
        if not os.path.exists(self.paths["query_labels"]):
            ...
            train_ann_path = "data/charades/annotations/charades_sta_train_pos.json"

...
def create_loaders(split, loader_configs, num_workers):
    ...
    for di,dt in enumerate(split):
        ...
        dsets[dt] = CharadesJSONDataset(loader_configs[di])
...
class CharadesJSONDataset(AbstractDataset):
    def __init__(self, config):
        ...
        ann_path = "data/charades/annotations/charades_sta_{}.json".format(self.split)
    ...
    def _get_data_path(self, config):
        ...
        root_dir = os.path.join(config.get("data_dir", ""), "preprocess_unsup_openimg_notfidf_yolo_0")
    ...
    def _preprocessing(self, anns, aux_ann_path):
        ...
        for qid,ann in enumerate(anns):
            query = ann["sentence"]
            vid = ann["vid"]
            spos,epos = ann["timestamp"]
            duration = float(vid2len[vid])
            new_anns[str(qid)] = {
                "timestamps": [float(spos) * duration, float(epos) * duration],
                "query": query,
    ...
    def generate_labels(self, config):
        ...
        if not os.path.exists(self.paths["query_labels"]):
            ...
            train_ann_path = "data/charades/annotations/charades_sta_train.json"
